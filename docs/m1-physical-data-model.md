...
- 币种：仅 USD
- 核心表数量：24
- 当前状态：设计完成；M1.1B 的 9 张企业身份与 SaaS 表已由 revision `0002` 实施，M1.1C 的 6 张商城表已由 revision `0003` 实施，M1.1D 的 5 张营销表已由 revision `0004` 实施，其余表尚未实施
...
受限信息，不保存支付凭证。支撑 SaaS Revenue、SaaS 新增付费、SaaS CAC/Attributed ROAS 和归因收入。
...
| `category_code` | ASCII binary `VARCHAR(64)` | 否 | 无 | M1 受控商品类别代码 |
...
- Check：状态集合、类别代码非空；首次发布和归档不得早于创建，archived 状态要求 `archived_at`。
...
- Check：币种 USD；所有业务时间不早于创建；paid/fulfilled/completed 必须有首次支付时间；fulfilled
  要求履约时间，completed 要求完成时间，cancelled 要求取消时间。履约和完成不得早于首次支付，两者
  同时存在时完成不得早于履约。completed 不强制履约时间，cancelled 可保留取消前的合法历史时间。
...
| `product_category_code` | ASCII binary `VARCHAR(64)` | 否 | 无 | 购买时类别快照 |
...
- Check：类别快照非空、`quantity > 0`、金额非负、币种 USD。
- `ix_order_item__product (product_id)`。
- `ix_order_item__category_order (product_category_code, commerce_order_id)` 支撑类别聚合。

不创建 `ix_order_item__order`；唯一索引 `(commerce_order_id, external_order_item_id)` 的左前缀已经
支持 order 查找，额外普通索引会完全冗余。
...
| `currency_code` | `CHAR(3)` | 否 | `USD` | 币种 |
| `reason_code` | ASCII binary `VARCHAR(64)` | 是 | `NULL` | 受控退款原因，不保存正文 |
...
- Check：状态集合、四项金额非负、总额等于组成合计、币种 USD；requested/pending 不带终态时间，
  succeeded/failed/cancelled 必须且只能有对应终态时间；处理和终态时间非空时不早于申请。
...
不创建 `ix_refund_alloc__refund`；唯一索引 `(commerce_refund_id, commerce_order_item_id)` 的左前缀已经
支持 refund 查找。
...
Attributed ROAS。直接支撑 `GQ-COM-005/008`、`GQ-MKT-003/006/007/008`、`GQ-XDM-001/002`。
...
**策略与覆盖**：渠道定义通常非敏感；`channel_code` 创建后 immutable，`channel_name` 作为显示名允许更新。
已被事实引用后不可删除或改变代码的历史语义，语义变化需创建新 channel code。unknown/unattributed 只
保存在归因结果中。支撑 CAC、Attributed ROAS、Attributed Revenue 和渠道趋势。
...
| `business_scope` | `VARCHAR(32)` | 否 | 无 | `saas/commerce`，冻结主体与权威事实域 |
...
- Check：business scope、状态集合；结束晚于开始；状态更新时间不早于创建；SaaS campaign 不得引用 merchant assignment。
...
- `ix_marketing_campaign__scope_status (business_scope, status)`。

**策略与覆盖**：名称、投放关系属于受限信息。活动状态可更新，创建时间不可覆盖；删除 `RESTRICT`。
若 merchant assignment 非空，其 organization 必须等于 `organization_id`，由应用和数据质量校验。
支撑 Activation Rate、CAC、Attributed ROAS、Attributed Revenue 和活动效率。直接支撑
`GQ-MKT-001/002/005/006/007`、`GQ-PRD-001/005/006/007/008`、`GQ-XDM-002/003/007`。

### 10.3 `campaign_daily_spend`

**用途与粒度**：保存一个活动在一个洛杉矶业务自然日的一版最终实际广告花费。更正只能追加新 revision，
不能原地更新旧 revision。

**主键与业务键**：主键 `campaign_daily_spend_id`；revision 唯一键
`(marketing_campaign_id, business_date, version_number)`。
...
| `version_number` | `SMALLINT UNSIGNED` | 否 | 无 | 从 1 开始递增的 revision 号 |
| `supersedes_campaign_daily_spend_id` | `BIGINT UNSIGNED` | 是 | `NULL` | version > 1 时引用被替代 revision |
| `spend_amount` | `DECIMAL(19,4)` | 否 | 无 | 该 revision 的当日最终实际花费 |
| `currency_code` | `CHAR(3)` | 否 | `USD` | 币种 |
| `finalized_at` | `DATETIME(6)` | 否 | 无 | 来源将该 revision 定稿的 UTC 时刻 |
| `is_test` | `BOOLEAN` | 否 | `FALSE` | 测试花费标记 |
| `recorded_at` | `DATETIME(6)` | 否 | 当前 UTC 时间 | revision 在 InsightOps 中可见的时间 |

**外键、约束和索引**：

- `fk_campaign_spend__campaign`、`fk_campaign_spend__supersedes`。
- `uq_campaign_spend__campaign_date_version (marketing_campaign_id, business_date, version_number)`。
- `uq_campaign_spend__supersedes (supersedes_campaign_daily_spend_id)` 防止 revision 分叉。
- Check：version 为正；version 1 不得 supersede，version > 1 必须 supersede；花费非负、币种 USD；
  `recorded_at >= finalized_at`。
- `ix_campaign_spend__date_recorded (business_date, recorded_at, is_test)`。

**策略与覆盖**：花费为受限商业信息，每行均为 final snapshot revision，不保存 provisional 状态，也没有
`updated_at`。数据库结构保留完整更正链；写入服务必须禁止 UPDATE/DELETE，并校验新 revision 与被替代行
属于同一 campaign/date 且 version 连续。快照查询必须先筛选 `recorded_at <= snapshot_cutoff`，然后在
可见 revision 中按 `(campaign, business_date)` 选择最大 `version_number`；禁止直接对全表 `MAX(version)`。
同一日期只能使用被选中的一行，不能把更正与原值相加。支撑实际花费、CAC、Attributed ROAS 和趋势。直接支撑
...
- Check：organization 与 consumer 恰好一个非空；touch type 和质量状态集合；满足
  `occurred_at <= received_at <= processed_at <= recorded_at`（processed 可空）的可见时间顺序。
...
- `ix_marketing_touch__visibility (recorded_at, quality_status)`。

**策略与覆盖**：事件 append-only，rejected 行保留用于质量审计但不参与归因。某次 attribution 只读取
`recorded_at <= source_data_cutoff_at` 的 accepted touch；cutoff 后到达的历史 touch 不回写已有结果，而是用
更晚 cutoff 追加新的 `attributed_conversion` 结果。不保存设备原始标识和原始载荷；身份连接属于敏感
信息。支撑 168 小时最后非直接触达、渠道分布和转化漏斗。直接支撑
`GQ-MKT-003/004/008`、`GQ-XDM-003`。

### 10.5 `attributed_conversion`

**用途与粒度**：保存一个权威业务转化在 `last_non_direct_168h_v1` 和一个 source-data cutoff 下的最终归因
结果。同一转化可以因 late-arriving touch 在更晚 cutoff 追加新结果，但旧结果不可覆盖。
...
| `selected_marketing_touch_id` | `BIGINT UNSIGNED` | 是 | `NULL` | 被选中的最终 non-direct 触达 |
...
| `reason_code` | `VARCHAR(64)` | 否 | 无 | 决策原因代码 |
...
| `source_data_cutoff_at` | `DATETIME(6)` | 否 | 无 | 本次归因可读取的 touch/spend 快照截止时间 |
...
- 三个唯一约束分别为 `(conversion_type, invoice_payment_attempt_id, model_version, source_data_cutoff_at)`、
  `(conversion_type, commerce_order_id, model_version, source_data_cutoff_at)`、
  `(conversion_type, platform_fee_charge_id, model_version, source_data_cutoff_at)`；MySQL 允许其他行的 NULL，
  不影响对应权威事实、模型和 cutoff 的唯一性。
...
- non-direct 要求 selected touch 和 channel 非空；direct 要求 selected touch 为空、channel 非空且 campaign
  为空；unknown 要求 selected touch/channel/campaign 全空。result 与 reason code 必须一致；
  `window_history_incomplete` 仅允许 `history_complete = FALSE`，其他 reason 要求完整历史。跨表 touch 的
  subject/channel/campaign 一致性及 direct channel 类型由写入服务和数据质量校验。
...
**策略与覆盖**：归因结果是 append-only 版本化派生事实；同一权威事实、模型版本和 cutoff 的重复计算必须
幂等，as-of 查询先按 `source_data_cutoff_at <= snapshot_cutoff` 选取最新结果。M1 Gold SQL 只能读取该表
中的最终结果，不得重新实现 `last_non_direct_168h_v1`。M1 不实现多触点。身份连接和收入关联敏感。支撑 CAC、Attributed ROAS、
...
**实施状态**：已由 Alembic revision `0003` 实施。
...
| CAC/Attributed ROAS | spend + attributed conversion + 权威转化事实 | 身份/收入类型分开、零分母、direct/unknown |
