...
Questions。所有问题引用 `insightcloud-business-definitions 1.0.1`，并映射到
...
- Activation 问题使用独立的 `activation_observation_as_of_utc=2025-07-01T07:00:00Z` 判断 14 日窗口
  是否成熟；不得用 2026 年 dataset snapshot cutoff 替代。
- Attribution 指标统一命名为 SaaS/Commerce Attributed ROAS，不与财务 ROI 混用；Gold SQL 读取已物化
  attribution，不重新实现 touch 选择算法。
...
| GQ-MKT-004 | 2025 年第二季度从营销触达到新增付费客户的同 cohort 漏斗如何，SaaS 和 Commerce 分别展示？ | L2 | 营销、SaaS、商城 | 漏斗分析 | touched cohort 固定为 Q2 至少一次 accepted、非测试 touch 的 distinct subject；首次付费主体必须属于该 cohort，且至少一条 qualifying touch 不晚于 conversion | touched subject 数；同 cohort first-payment subject 数 | touch、campaign、channel、attributed_conversion、organization、consumer、payment_attempt、order | 2025-04-01 至 2025-07-01；只使用精确 first-payment conversion；测试链完整排除 | 两个身份范围的同 cohort 阶段计数，第二阶段不得大于第一阶段 | 否；已明确不混合身份，漏斗只展示基础计数 | P04、P12、P13 |
...
| GQ-MKT-008 | 给定 2025 年第二季度的转化，direct、unknown/unattributed、晚到触达重归因和历史渠道失效边界分别是什么？ | L3 | 营销、SaaS、商城 | 跨域归因分析 | 读取物化归因修订，展示历史不足、coverage equality、late-arriving 双 cutoff、inactive historical channel 和 direct | boundary case、cutoff、result/reason、selected touch/channel、latest revision | attributed_conversion、touch、campaign、channel、权威转化事实、organization、consumer | 转化期 2025-04-01 至 2025-07-01；按 source-data cutoff 展示物化修订，不在 Gold SQL 重算 168 小时算法 | 按边界、业务域、转化和 cutoff 排序的逐笔归因修订表 | 否；规则已冻结，不能把 unknown 伪装成 direct，渠道当前 status 不追溯改变历史归因 | P12、P13、history boundary、late-arriving、inactive historical channel |
...
| GQ-PRD-005 | 2025 年第二季度不同注册来源和套餐的 organization Activation Rate 有何差异？ | L2 | 企业增长、营销、SaaS | 分群比较 | 比较成熟 cohort 的激活表现 | Activation Rate | organization、member、campaign、merchant、order、subscription | 注册期 2025-04-01 至 2025-07-01，观察至各自第 14 日 | 来源与套餐交叉分组表，含分子分母 | 是；当前模型没有注册来源归因，首次付费归因不得用于推断注册来源 | P09、P12 |
...
| GQ-XDM-002 | 2025 年第二季度各 merchant 的营销花费、Attributed Commerce Revenue 和 Commerce Attributed ROAS 如何？ | L2 | 商城、营销 | 跨域归因分析 | 连接商家归属、活动成本和平台交易收入 | Attributed Commerce Revenue、Commerce Attributed ROAS、实际花费 | merchant、organization、campaign、daily_spend、attributed_conversion、platform_fee_charge | 2025-04-01 至 2025-07-01 | merchant 比较表，含未归因披露 | 否；先按活动/收入事实聚合再连接 merchant | P07、P12、P13 |
