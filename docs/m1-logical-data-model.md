...
- **测试与敏感性**：自身、invoice、subscription 或 organization 为测试时排除；支付提供商标识和金额
  属于高敏受限信息，不保存支付凭证。
- **依赖指标**：SaaS Revenue、SaaS 新增付费 organization、SaaS CAC、归因 SaaS Revenue、SaaS ROAS。
...
- **依赖指标**：Platform Transaction Revenue、Commerce Revenue、归因 Commerce Revenue、Commerce ROAS。
...
- **依赖指标**：CAC、ROAS、Attributed Revenue、新增付费客户数和渠道趋势。
...
- **依赖指标**：Activation Rate、CAC、ROAS、Attributed Revenue、活动效率分析。
...
- **业务用途、粒度和主键**：一行代表一个 campaign 在一个业务自然日确认的实际广告花费；业务主键为
  campaign ID 与业务日期。
- **关系与状态**：多对一关联 campaign；状态区分 provisional、confirmed、corrected，正式指标仅使用
  截止快照中有效的 confirmed/corrected 最终值。
- **时间语义**：业务日期按 `America/Los_Angeles` 划分，同时保留实际花费确认 UTC 时刻。
- **事实类型和报表时间**：自然日周期事实，不是累计快照；更正不能与原值重复相加。
- **测试与敏感性**：活动或花费行测试时排除；花费属于受限商业信息。
- **依赖指标**：CAC、ROAS、营销花费和渠道/活动效率趋势。
...
- **时间语义**：区分接触发生、接收和处理时间；168 小时窗口使用真实发生时刻。
- **事实类型和报表时间**：事件事实；同时间触达使用稳定来源事件 ID 决定顺序。
- **测试与敏感性**：触达、渠道、活动或主体测试时排除；设备和营销标识属于个人敏感信息。
- **依赖指标**：最后非直接触达归因、Attributed Revenue、CAC、ROAS、新增付费客户渠道分布。
...
- **事实类型和报表时间**：版本化派生事实，但结果必须能追溯至原触达和权威转化；V1 模型固定为
  168 小时最后非直接触达，候选窗口为 `[转化时刻 − 168 小时, 转化时刻]`。
- **测试与敏感性**：触达、转化、渠道、活动或主体任一测试时不得形成正式归因；身份连接属于敏感信息。
- **依赖指标**：Attributed Revenue、CAC、ROAS、新增付费客户数及 unattributed 披露。
...
| ROAS | daily spend、attributed conversion、权威收入事实 | 花费确认时间与收入转化时间 | confirmed spend；明确 SaaS 或 Commerce 收入 | 多触达复制收入、跨收入类型相加 | 168 小时归因、收入类型命名、零花费为 null |
...
| 5 | 活动 ROAS 明显下降 | campaign spend、attributed revenue 和明确收入类型 |
