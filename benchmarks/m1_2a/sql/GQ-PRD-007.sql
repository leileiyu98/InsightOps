WITH cohort AS (
    SELECT
        org.organization_id,
        CASE WHEN org.registered_at < :jun_start THEN '2025-05' ELSE '2025-06' END
            AS cohort_month,
        org.registered_at,
    ...
    WHERE org.registered_at >= :may_start AND org.registered_at < :jul_start
...
counts AS (
    SELECT
        ...
        c.cohort_month,
        c.is_mature,
        COUNT(e.condition_name) AS condition_count
    FROM cohort AS c
    LEFT JOIN condition_events AS e ON e.organization_id = c.organization_id
    GROUP BY c.organization_id, c.cohort_month, c.is_mature
)
SELECT
    cohort_month,
    COUNT(*) AS registered_organization_count,
