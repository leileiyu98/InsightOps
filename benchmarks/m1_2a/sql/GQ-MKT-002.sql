...
spend_by_channel AS (
    ...
    JOIN merchant AS m ON m.merchant_assignment_id = c.merchant_assignment_id
    ...
    WHERE s.revision_rank = 1
      ...
      AND c.is_test = 0 AND ch.is_test = 0 AND m.is_test = 0 AND org.is_test = 0
...
ranked_conversions AS (
    SELECT
        ...
        ac.recorded_at,
        ROW_NUMBER() OVER (
            PARTITION BY ac.commerce_order_id
    ...
    WHERE ac.conversion_type = 'commerce_first_payment'
...
customers_by_channel AS (
    SELECT
        ...
        COUNT(DISTINCT ac.consumer_id) AS new_customers
